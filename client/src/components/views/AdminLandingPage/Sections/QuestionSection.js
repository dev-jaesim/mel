...
function QuestionSection() {
  ...
  const updateQuestion = async (values) => {
    ...
    await axios.put(`${QUESTION_SERVER}/update`, variables);

    await loadQuestions(keywords);
    setIndividual(null);
    setSubject(null);
    setIndividualQuestionModalVisible(false);
  };

  const popConfirm = async (e) => {
    await axios.delete(`${QUESTION_SERVER}/delete`, {
  ...
  const addQuestion = async (values) => {
    await axios.post(`${QUESTION_SERVER}/add`, values);
    await loadQuestions(keywords);
    setSubject(null);
    setQuestionModalVisible(false);
  };
